...
| `--color-bg-secondary` | `#f8f5fc` | Fundo alternativo (lavanda claro) |
| `--color-bg-card` | `rgba(255, 255, 255, 0.8)` | Fundo de cards |
| `--color-text-primary` | `#1a0a2e` | Texto principal (roxo muito escuro) |
| `--color-text-secondary` | `#4a3f5c` | Texto secundário |
| `--color-text-muted` | `#6b5d7a` | Texto atenuado |
...
O `@theme` é uma diretiva do Tailwind CSS v4 que define design tokens disponíveis como utilitários Tailwind (ex: `bg-primary`, `text-text-muted`). NÃO é um sistema de temas múltiplos — existe apenas um conjunto de variáveis.
